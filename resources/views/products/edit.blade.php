                            ...
                            <div class="form-group">
                                ...
                                <select class="select" name="categories[]" multiple required>
                                    ...
                                    <option value="{{ $category->id }}" {{ in_array( $category->id,
                                        old('category_ids', $product->categories->pluck('id')->toArray())
                                        ) ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                    @endforeach
                        ...
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <select class="select" name="main_unavailable_sizes[]" multiple>
                                    @foreach (getAllSizes() as $size)
                                    <option value="{{ $size->id }}" {{ in_array( $size->id,
                                        old(
                                        'main_unavailable_sizes',
                                        $unavailableSizesByColor[$mainColor->id] ?? []
                                        )
                                        ) ? 'selected' : '' }}
                                        >
                                        {{ $size->name }}
                        ...
                        <div class="col-lg-12">
                            ...
                            <div id="colorVariants">
                                ...
                                ->where('color_id', $color->id);
                                        $unavailableSizesForColor = $unavailableSizesByColor[$color->id] ?? [];
                                ...
                                <div class="card mt-3 p-3 variant-block" data-index="{{ $loopIndex }}">
                                    <div class="row">
                                        ...
                                        <!-- Unavailable Sizes for this color -->
                                        <div class="col-lg-12 mt-3">
                                            <label>Unavailable Sizes for this Color</label>
                                            <select class="select" name="colors[{{ $loopIndex }}][unavailable_sizes][]"
                                                multiple>
                                                @foreach(getAllSizes() as $size)
                                                <option value="{{ $size->id }}" {{ in_array($size->id,
                                                    old("colors.$loopIndex.unavailable_sizes",
                                                    $unavailableSizesForColor)) ? 'selected' : '' }}>
                                                    {{ $size->name }}
                                                </option>
                                                @endforeach
                                            </select>
...
<script>
    ...
    const updateEndpoint = '{{ route("admin.products.update", $product->id) }}';
    ...
    $(document).ready(function () {
        ...
        $('#addColorBtn').on('click', function () {
            ...
            const variantHtml = `
                <div class="card mt-3 p-3 variant-block" data-index="${colorIndex}">
                    <div class="row">
                        <div class="col-lg-6">
                            <label>Color Name</label>
                            <input type="text" name="colors[${colorIndex}][name]" class="form-control" required>
                        </div>
                        <div class="col-lg-6">
                            <label>Color Code (e.g. ff0000)</label>
                            <input type="text" name="colors[${colorIndex}][code]" class="form-control" required>
                        </div>
                        ...
                        <div class="col-lg-12 mt-3">
                            ...
                            <select
                                class="select"
                                name="colors[${colorIndex}][unavailable_sizes][]"
                                multiple
                            >
                                @foreach (getAllSizes() as $size)
            ...
            `;
             ...
             // ✅ CREATE jQuery element ONCE
    const $variant = $(variantHtml);

    // ✅ APPEND ONCE
    $('#colorVariants').append($variant);

    // ✅ INIT ON THE SAME ELEMENT
    initSelects($variant);
    setupUploadBox($variant.find('.color-main-upload'));
    setupUploadBox($variant.find('.upload-box').last());
        });
